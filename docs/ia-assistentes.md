...
Landing **estática** em React/Vite para o produto **Protocolo Jurisprudência Inteligente**. Não existe API server-side no repo; não inventar endpoints ou integração Gemini/Google a menos que o código já mostre isso.
...
5. Se precisar adicionar nova variação visual global, atualizar junto `src/index.css` e `docs/design/design-system.md`.
